...
import { SectionBlock, MarkdownText, ActionBlock, ButtonElement, SlackText, BlockMessage } from '../../models/slack-block-models';
...
export default class TripHelpers {
  ...
  static async getApprovalOrDeclineMessage(trip: TripRequest, status: ITripState,
    channelId: string, actorId: string) {
    const [header, main, footer] = [new SectionBlock(), new SectionBlock(), new SectionBlock()];
