...
import database, { TripRequest, Department, Provider, Homebase } from '../../database';
...
import { departmentDataAttributes } from '../../services/DepartmentService';
...
export class TripService extends BaseService<TripRequest, number> {
  ...
  constructor(model = database.getRepository(TripRequest)) {
    ...
    this.defaultInclude = [
      { model: Provider, include: ['user'] },
      { model: Homebase, include: ['country'] },
  ...
  async getTrips(pageable: IPageable, where: any, homebaseId: number) {
    ...
    const defaultOptions = this.createFilter({ ...where, homebaseId });
