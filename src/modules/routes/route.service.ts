...
class RouteService extends BaseService <Route, number> {
  ...
  async createRouteBatch(
    ...
    const routeBatch: RouteBatch = await routeBatchService.createRouteBatch(routeBatchObject);
    appEvents.broadcast({
      name: routeEvents.newRouteBatchCreated,
      data: { botToken, routeBatch },
    });
    return routeBatch;
  }
  ...
  convertToSequelizeOrderByClause(sort: { predicate: string, direction: string }[]): any[] {
