...
class CountryService extends BaseService <Country, number>{
  ...
  async deleteCountryByNameOrId(id: number = -1,
    ...
    const findBy: { id: number, name: string } = { id: -1, name: '' };
    if (id) findBy.id = id;
    if (name !== '') findBy.name = name;
    const country = await this.model.findOne({
      where: {
        [Op.or]: [{ id: findBy.id }, { name: { [Op.iLike]: `${findBy.name}%` } }],
      },
    });
    HttpError.throwErrorIfNull(country.get(), `Country named '${name}' was not found`, 404);
